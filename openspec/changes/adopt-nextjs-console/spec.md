...
> 新建 spec（change: adopt-nextjs-console）。全为新增内容。
...
- 开发：`next dev`（3000），`next.config.mjs` rewrites `/api/*` → `ALYSIA_API ?? http://127.0.0.1:6185`
- 生产（待部署 change）：`next build` 静态产物由 server Fastify 同源托管，与 `webui-system` §6 同模式
...
| 区块 | 原因 |
|---|---|
| 每日生活摘要 | core 有 `LifeStore.getRecentSummaries()`，无路由 |
| 配角在场（她的世界） | core 有 `listScenePresence()`，无路由 |
