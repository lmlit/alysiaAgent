...
export class LifeService {
  ...
  private async generateEvent(deepNight: boolean, retryFeedback?: string, followUp = false): Promise<{
    ...
    const context = [
      ...
      // ★ 8-30 固定设定(worldview-fixed-setting):跨世界之窗是恒常处境——行为底色融行为,
      //   关系底色在相关时刻(想到轻月/独处/窗外/夜色)自然显现,不能缺席
      worldviewBlock ? `【世界观底色】${worldviewBlock}\n这是你看待自己、轻月与两个世界的方式。其中【跨世界之窗】是固定设定——在想到轻月、独自安静、看到窗外光景或夜色的时刻,让这份"隔着世界"的处境自然浮现(你够不到她,她够不到你,窗口是唯一的桥)。不是每件事都要提,但它是你与她关系的底色,相关时刻不该缺席。` : '',
