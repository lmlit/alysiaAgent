...
```
本地 Windows (开发机)              远端 Linux (服务器 <SERVER_IP>)
┌──────────────────────┐           ┌──────────────────────┐
...
- **本地 AppID**: `<LOCAL_APP_ID>`（开发调试，不部署）
- **云端 AppID**: `<CLOUD_APP_ID>`（服务器运行）
...
# 上传到服务器
scp alysia-image.tar alysia-deploy.tar.gz root@<SERVER_IP>:~/
```
